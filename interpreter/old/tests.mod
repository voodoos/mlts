...
%                   ) V.
%
%% The original evaluator failed when a nab bound variable was vacuous.  This is fixed.
%test 80 V :- eval ((fixpt test\ lam term\ match term [(nab X\ (ab W\ W) ==> (i 1))]) arobase (abt W\W)) V.
%
%test 81 V :- eval ((fixpt test\ lam term\ match term [((ab W\ W) ==> (i 1))]) arobase (abt W\W)) V.
%

main :- (test N V, term_to_string N Str, print "Value for ", print Str, print ": ",
